...
if (s:conceal_aggressively == 1)
   ...
   syntax region org_underlinematchgroup=org_border_undl start="[^ \\]\zs_\|\(\(^\|[^\\]\)\zs\(\\\\\)\+\)\zs_\|\(^\|[^\\]\)\@<=_\S\@="        end="[^ \\]\zs_\|\(\(^\|[^\\]\)\zs\(\\\\\)\+\)\zs_\|[^\\]\zs_\S\@="     concealends oneline
...
" Tables: {{{1
syn match org_table /^\s*|.*/
            \ contains=org_timestamp,org_timestamp_inactive,hyperlink,org_table_separator,org_table_horizontal_line
syn match org_table_separator /\(^\s*|[-+]\+|\?\||\)/ contained
hi def link org_table_separator Type

" Hyperlinks: {{{1
syntax match hyperlink "\[\{2}[^][]*\(\]\[[^][]*\)\?\]\{2}"
            \ contains=hyperlinkBracketsLeft,hyperlinkURL,hyperlinkBracketsRight
            \ containedin=ALL
if (s:conceal_aggressively == 1)
...
syntax match org_list_item /.*$/
            \ contained
            \ contains=org_subtask_percent,org_subtask_number,org_subtask_percent_100,org_subtask_number_all,org_list_checkbox,org_bold,org_italic,org_underline,org_code,org_verbatim,org_timestamp,org_timestamp_inactive,org_list_def
...
if exists('g:loaded_SyntaxRange')
  ...
  call SyntaxRange#Include('#+BEGIN_SRC c++', '#+END_SRC', 'cpp', 'comment')
